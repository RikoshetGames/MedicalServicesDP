...
В файле присутствуют настройки is_superuser и is_staff. В зависимости от значения True или False создаётся админ, можератор или обычный пользователь.
Для создания пользователя воспользуйтесь командой `python manage.py csu` ▶️.
Примечание: для создания модератора необходимо указать is_superuser = False и is_staff = True, после чего через админку суперпользователя назначить модератору группу moderator
